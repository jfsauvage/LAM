PRO init_ao_harmoni, varnoise_wfs = varnoise_wfs, lambda_imaging = lambda_imaging, $
                  dimpup = dimpup, anglezenith = anglezenith, adim = adim, $
                  ind_turb_case = ind_turb_case, beta_tab = beta_tab, L0 = L0, ao_flavour = ao_flavour

if ao_flavour EQ 'SCAO' then begin
  sep = 0.0                      ; [arcmin] 0.0 for SCAO
  nb_gs = 1.                    ; Number of Guide Stars, 1 for SCAO
  LGS_H = 0; 9e4              ; Altitude for LGS (0 for infinity)

  ; Reconstruction turbulence a priori : nombre et altitudes de couches
  recons_h = [0.0]              ; supposed altitudes for estimation
  cn2recons = [1.0]             ; assumed CN2 for estimation
endif

if ao_flavour EQ 'LTAO' then begin
  sep = 0.5                      ; [arcmin] 0.0 for SCAO
  nb_gs = 6.                    ; Number of Guide Stars, 1 for SCAO
  LGS_H = 9e4              ; Altitude for LGS (0 for infinity)

  ; Reconstruction turbulence : nombre et altitudes de couches
  recons_h = [0.0, 10000.] ; [0.0, 1000., 5000., 10000.]              ; supposed altitudes for estimation
  cn2recons = [0.5, 0.5] ; [0.25, 0.25, 0.25, 0.25]             ; assumed CN2 for estimation
endif
...
; sep = 0.0                      ; [arcmin] Careful > 1.6 arcmin to ensure NO unseen area
; nb_gs = 1.                    ; Number of Guide Stars
; LGS_H = 0; 9e4              ; Altitude for LGS (0 for infinity) 

; -- Simulation photometry --
lambda_wfs = 0.700e-6                                                             ; wfs wavelength [m]
...
turb_profile = readfits('/home/jfsauvage/HARMONI/SimuFourier/profil_turbulent_eso.fits')
